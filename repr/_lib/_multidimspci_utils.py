...
# Patch: np.math.gamma -> math.gamma (NumPy 2.0+ removed np.math)
...
def ellipsoid_volume(covariance_matrix, r):
    ...
    constant_cd = np.pi ** (num_r / 2) / math.gamma(num_r / 2 + 1)
...
def adjust_alpha_t(alpha_t, alpha, errs, gamma=0.005, method="simple"):
    if method == "simple":
        ...
        t = len(errs)
        errs = np.array(errs)
        w_s_ls = np.array([0.95 ** (t - i) for i in range(t)])  # Furthest to Most recent
...
def binning(past_resid, cov_mat_est, alpha, bins=5):
    ...
    Description:
        Compute the beta^hat_bin from past_resid, by breaking [0,alpha] into bins (like 20).
    """
...
def binning_use_RF_quantile_regr(
    quantile_regr, cov_mat_est, Xtrain, Ytrain, feature, beta_ls, sample_weight=None
):
