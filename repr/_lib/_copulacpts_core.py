...
# Patches:
#   - `from copulae.core import pseudo_obs` replaced with local import.

import numpy as np
import torch

from ._copulacpts_utils import pseudo_obs, search_alpha
...
class copulaCPTS:
    def __init__(self, model, cali_x, cali_y):
        ...
        Args:
            model: object with a ``predict(x)`` method returning a torch.Tensor.
            cali_x: calibration inputs (array or index).
            cali_y: calibration ground-truth (array).
        """
        self.model = model
    ...
    def predict(self, X_test, epsilon=0.1):
        ...
        mapping = {
            i: sorted(self.nonconformity[:, i].tolist())
    ...
    def calc_area(self, radius):
        return sum([np.pi * r**2 for r in radius])

    def calc_area_l1(self, radius):
        return sum([2 * r**2 for r in radius])

    def calc_area_3d(self, radius):
        return sum([4 / 3.0 * np.pi * r**3 for r in radius])
    ...
    def calc_coverage(self, radius, y_pred, y_test):
        testnonconformity = (
            torch.norm((y_pred - y_test), p=2, dim=-1).detach().numpy()
        )
        circle_covs = []
